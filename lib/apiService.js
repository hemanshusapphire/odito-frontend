...
class ApiService {
  ...
  async getAccessibilityIssues(filters = {}) {
    ...
    console.log('🔍 Getting accessibility issues:', { endpoint, filters });

    try {

      const response = await this.request(endpoint);

      console.log('✅ Accessibility issues response:', response);

      return response;

    } catch (error) {

      console.error('❌ Accessibility issues error:', error);
