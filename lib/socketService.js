...
class SocketService {
  ...
  // ── Lead events (Phase 3B) ───────────────────────────────────
  // Same project-{projectId} room, same generic plumbing as every event
  // group above. Emitted by wordPressSubmissionService.js's
  // notifyLeadCreated() the moment a WordPress form submission becomes a
  // real Lead — payload is deliberately just { projectId, leadId }, never
  // the lead's PII, so listeners invalidate/refetch rather than read the
  // event payload directly.
  onLeadCreated(callback) { this.addEventListener('lead:created', callback); }
  offLeadCreated(callback) { this.removeEventListener('lead:created', callback); }

  // ── Bulk Social Upload import progress (Phase 5) ─────────────────────
  // Same project-{projectId} room, same generic plumbing as every group
  // above. Emitted only by the backend bulkImportExecutor (the
  // authoritative execution path) + the stale-import sweeper. Payloads
  // are operational metadata only — counts, ids, row number, status.
  onBulkImportStarted(callback) { this.addEventListener('bulk-import:started', callback); }
  onBulkImportProgress(callback) { this.addEventListener('bulk-import:progress', callback); }
  onBulkImportCompleted(callback) { this.addEventListener('bulk-import:completed', callback); }
  onBulkImportError(callback) { this.addEventListener('bulk-import:error', callback); }

  offBulkImportStarted(callback) { this.removeEventListener('bulk-import:started', callback); }
  offBulkImportProgress(callback) { this.removeEventListener('bulk-import:progress', callback); }
  offBulkImportCompleted(callback) { this.removeEventListener('bulk-import:completed', callback); }
  offBulkImportError(callback) { this.removeEventListener('bulk-import:error', callback); }
