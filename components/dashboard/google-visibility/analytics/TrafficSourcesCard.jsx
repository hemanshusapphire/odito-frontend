...
import { formatCompactNumber } from '@/lib/analyticsChartConfig'
...
function DonutTooltip({ active, payload }) {
  ...
  return (
    <div className="rounded-lg border bg-popover px-3 py-2 shadow-md text-xs">
      ...
      <div className="text-muted-foreground mt-1 tabular-nums">{value.toLocaleString()} users</div>
    </div>
  )
}

/**
 * Donut breakdown of users by acquisition channel, with a centered total
 * and a value/percentage legend - same shell as
 * business-profile/CustomerActionsCard.jsx.
 */
export default function TrafficSourcesCard({ sources = [], loading = false, rangeLabel = 'the selected period' }) {
  const total = sources.reduce((sum, s) => sum + (s.value || 0), 0)

  return (
    <Card className="p-6">
      <h3 className="text-sm font-semibold">Traffic Sources</h3>
      <p className="text-xs text-muted-foreground mt-0.5">Users by acquisition channel, {rangeLabel}</p>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 mt-4">
          <div className="relative h-40 w-40 shrink-0">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              <span className="text-xl font-bold tabular-nums">{formatCompactNumber(total)}</span>
